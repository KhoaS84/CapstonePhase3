...
| **9** | **Tích hợp trực tiếp SDK boto3 (Bedrock)** | Sửa `product_reviews_server.py` để sử dụng trực tiếp SDK `boto3` (Converse API) của AWS thay thế cho thư viện `OpenAI`. Loại bỏ được LiteLLM Proxy trên EKS, giảm 1 hop mạng trung gian, tối ưu hóa độ trễ (Latency) và nâng cao độ tin cậy. | `2`          | **High** (Tối ưu SLA & Độ trễ) | Backlog (Tuần 2) |
