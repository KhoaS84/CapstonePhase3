...
* [ ] Tính năng tóm tắt review trên storefront chạy bằng model thật `gpt-4o-mini` (không bị mock).
* [ ] File `deploy/values-aio-llm.yaml` được cập nhật chính xác và commit lên repo.
* [ ] Traces trên Jaeger ghi nhận đầy đủ luồng đi và thời gian xử lý của API LLM thật.
* [ ] Bảng số liệu Latency & Cost được điền đầy đủ vào Mục 1 trong file `AI_BASELINE_EVAL.md`.
...
### Tiêu chí nghiệm thu (Acceptance Criteria)
* [ ] Mã nguồn script/code eval được commit vào thư mục dự án (vd: `repro/` hoặc `scripts/`).
